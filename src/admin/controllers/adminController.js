...
const dashboardSettings = {
  companyName: "Bike Admin Pro Pvt Ltd",
...
export async function deliveryPartners(req, res) {
  try {
    const [
      ...
      supabase.from("users").select("*").eq("is_delivery_partner", true),
      supabase.from("orders").select("*"),
      supabase.from("earnings").select("*"),
    ]);
    if (usersError || ordersError || earningsError) {
      console.error("[admin.deliveryPartners] fetch failed", usersError || ordersError || earningsError);
    }

    const allOrders = safeData(orders);
    const pendingOrders = allOrders.filter((order) => (order.status || "").toLowerCase() === "pending");
    const now = Date.now();
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const oneWeekMs = 7 * 24 * 60 * 60 * 1000;

    const partners = safeData(users).map((user) => {
      const state = partnerState[user.id] || {};
      const assignedOrder =
        allOrders.find((order) => (order.assigned_user_id || order.assignedUserId) === user.id) || null;
      const currentOrderId = state.currentOrderId || assignedOrder?.id || null;
      const isBusy = Boolean(currentOrderId) && !state.disabled;
      const isOnline = !state.disabled && (state.online ?? user.is_online);
      const status = state.disabled ? "offline" : isBusy ? "busy" : isOnline ? "online" : "offline";
      const partnerEarnings = safeData(earnings).filter((item) => item.userId === user.id);
      const totalEarnings = partnerEarnings.reduce((sum, item) => sum + Number(item.amount || 0), 0);
      const todayEarnings = partnerEarnings
        .filter(
          (item) =>
            new Date(item.createdAt || item.created_at || now).getTime() >= todayStart.getTime()
        )
        .reduce((sum, item) => sum + Number(item.amount || 0), 0);
      const weeklyEarnings = partnerEarnings
        .filter((item) => now - new Date(item.createdAt || item.created_at || now).getTime() <= oneWeekMs)
        .reduce((sum, item) => sum + Number(item.amount || 0), 0);

      const deliveries = allOrders.filter((order) => (order.assigned_user_id || order.assignedUserId) === user.id);
      const completed = deliveries.filter((order) => (order.status || "").toLowerCase() === "completed").length;
      const cancelled = deliveries.filter((order) =>
        ["rejected", "cancelled"].includes((order.status || "").toLowerCase())
      ).length;
      const totalDeliveries = deliveries.length;
      const successRate = totalDeliveries ? Math.round((completed / totalDeliveries) * 100) : 0;
      ...
      return {
        ...
        earningsWeekly: weeklyEarnings,
        earningsTotal: totalEarnings,
        rating: Number((4.2 + ((completed % 7) * 0.1)).toFixed(1)),
        totalDeliveries,
        successRate,
        avgDeliveryTime: totalDeliveries ? 22 + (totalDeliveries % 9) : 0,
        completed,
        cancelled,
      };
    });
    ...
    const stats = {
      ...
      active: partners.filter((p) => p.status === "online").length,
    ...
    return res.status(500).send("Unable to load delivery partners");
  }
}
...
export async function maintenance(req, res) {
  try {
    const { data, error } = await supabase.from("bikes").select("*");
    if (error) {
      console.error("[admin.maintenance] fetch failed", error);
    }
